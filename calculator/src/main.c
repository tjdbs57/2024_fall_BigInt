...
int main() {
    bigint *n = NULL, *rand_num = NULL;

    // 상한값 설정 (n = 1000)
    bi_set_by_string(&n, NON_NEGATIVE, "3e8", 16);

    // [2, n-2] 범위에서 랜덤 값 생성
    bi_set_random_in_range(&rand_num, "2", n);

    // 결과 출력
    printf("Random number in range [2, n-2]: ");
    bi_show_hex(rand_num);
